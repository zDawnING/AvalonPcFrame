define(['avalon',
	'text!components/modules/city_three_select_pc2/city_three_select_pc2.html',
	'china',
	'css!components/modules/city_three_select_pc2/city_three_select_pc2.css',
], function(avalon, template,chinaArea) {
	var vm = {}; 
	//定义为组件
	avalon.component("ms:citythreeselect2", {
		$template: template,
		/**
		...
		one_city_change:function(){
			...
			if(vm.one_city_index>=0){
				...
				document.getElementById("two_city_select_s").selectedIndex = 0;
				document.getElementById("three_city_select_s").selectedIndex = 0;
			}
		},
		//二级城市列表
		two_city:[],
		...
		set_area1:function(area1){
			...
			if( !area1 ){
				document.getElementById("one_city_select_s").selectedIndex = 0;
				document.getElementById("two_city_select_s").selectedIndex = 0;
				document.getElementById("three_city_select_s").selectedIndex = 0;
				return ;
			}
			...
			for(var i = 0;i<vm.one_city.size();i++){
				if( vm.one_city[i].name == area1 ){
					vm.one_city_index = i;
//					document.getElementById("one_city_select_s").options[i+1].selected = true;
					document.getElementById("one_city_select_s").selectedIndex = i+1;
					vm.two_city = chinaArea.china.province[vm.one_city_index].city;
					return ;
				}
			}
		...
		set_area2:function(area2){
			...
			for(var i=0;i<vm.two_city.size();i++){
				if( vm.two_city[i].name == area2 ){
					vm.two_city_index = i;
//					document.getElementById("two_city_select_s").options[i+1].selected = true;
					document.getElementById("two_city_select_s").selectedIndex = i+1;
					vm.three_city = vm.two_city[vm.two_city_index].county; 
					return ;
				}
			}
...
// 					document.getElementById("three_city_select_s").options[i+1].selected = true;
					document.getElementById("three_city_select_s").selectedIndex = i+1;
					return ;
				}
			}
		},	
	...
	avalon.citythreeselect2_module_vm = vm;
	return avalon;
});